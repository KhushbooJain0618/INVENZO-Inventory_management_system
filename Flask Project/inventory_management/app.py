...
@app.route('/api/order_management', methods=["POST"])
def order_management_api():
    data = request.get_json()
    if not data:
        return jsonify({"error": "No input data provided"}), 400

    required_fields = [
        "item_id", "item_name", "item_category", "quantity", "cost_per_unit", "total_cost",
        "supplier_name", "supplier_email", "supplier_phone", "order_status", "user_id"
    ]
...
@app.route('/api/order_management/<int:id>', methods=['PUT'])
def update_order_item(id):
    item = InventoryItem.query.get_or_404(id)  # Get item or return 404

    data = request.get_json()

    # Validate the incoming data
    try:
        item.item_id = int(data.get("item_id", item.item_id))
        ...
        item.total_cost = float(data.get("total_cost", item.total_cost))
        ...
        item.supplier_phone = data.get("supplier_phone", item.supplier_phone)
        item.order_status = data.get("order_status", item.order_status)
        item.notes = data.get("notes", item.notes)
        item.barcode_number = data.get("barcode_number", item.barcode_number)
        item.user_id = int(data.get("user_id", item.user_id))

        db.session.commit()
        return jsonify({"message": "Item updated successfully"}), 200

    except ValueError as e:
        return jsonify({"error": "Invalid input type. Make sure all numeric fields are correct."}), 400
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
@app.route('/api/order_management/<int:id>', methods=["DELETE"])
def delete_inventory_item(id):
    item = InventoryItem.query.get(id)  # Use get() instead of get_or_404 to debug
    if not item:
...
# for barcode page:
@app.route('/api/order_management/<int:item_id>', methods=["GET"])
...
def login_user():
    ...
    if user and user.check_password(password):  # Check if password matches
        # Save user session (could use JWT tokens instead of sessions if preferred)
        session['user_id'] = user.id
        session['username'] = user.username
        return jsonify({'message': 'Login successful!'}), 200  # Success message
    else:
...
def update_user_profile(id):
    user = User.query.get_or_404(id)  # Get user by ID
    ...
    # Optional: Allow password change (ensure hashed password)
